  ...
  constructor(
    ...
    private httpService: HttpService) {
      this.formPanel = fb.group({
        ...
        Corr_SEC: ['', [Validators.required]],
        HH_Skip: ['', [Validators.required]],
        Rec_Action: ['', [Validators.required]],
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      .then((path) => {
          ...
          if(this.platform.is('android')) {
            this.file.checkDir(this.file.externalRootDirectory, 'ob_val').then(response => {
              console.log('Directory exists'+response);
              this.moveToFile(imagePath,imageName,type)
            }).catch(err => {
              console.log('Directory doesn\'t exist'+JSON.stringify(err));
              this.file.createDir(this.file.externalRootDirectory, 'ob_val', false).then(response => {
...
moveToFile(imagePath,imageName,type){
  ...
  this.file.moveFile(imagePath, imageName, this.file.externalRootDirectory + "ob_val/", panel_code+"_"+'HSE'+".jpeg")
  .then(newFile => {
    switch(type){
      case 'House_Image':
...
getTrustImg(imageSrc){
  ...
  watchNetwork(){
   ...
   sync(){
    ...
    this.uploading = true
    this.httpService.post(this.url+"/ob_vals",this.formPanel.value,false).timeout(10000).subscribe(data => {
  ...
  uploadPhoto(value,type){
    this.uploading = true
    this.httpService.patch(this.url+"/ob_vals/"+this.irfObj['id']+"/upload",value,false).timeout(10000).subscribe(data => {
