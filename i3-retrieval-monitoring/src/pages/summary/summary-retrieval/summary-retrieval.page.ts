...
@Component({
  selector: "app-summary-retrieval-page",
  templateUrl: "./summary-retrieval.page.html",
})
export class SummaryRetrievalPage implements OnInit {
  ...
  getData() {
    this.platform.ready().then(() => {
      if (this.selectedTable == 'eop_purchase') {
        this.getRetrievalDataPurchaseEOP()
      } else if (this.selectedTable == 'hhp_purchase') {
        this.getRetrievalDataPurchaseHHP()
      } else if (this.selectedTable == 'retrieval') {
  ...
  getRetrievalDataPurchaseEOP() {
    this.sqliteEopService.searchByDate(this.date_start, this.date_end).then(
      (data: any) => {
        ...
        console.log("serach data", result.length);
        this.requests = result
        this.statuses = this.grouBy(result, "period_code")
        Object.keys(this.statuses).forEach(e => {
          this.statuses[e]["panel_code"] = this.grouBy(this.statuses[e], "panel_code")
        })
