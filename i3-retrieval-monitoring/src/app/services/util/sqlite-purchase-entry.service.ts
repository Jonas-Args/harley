      ...
      this.sqlite
        ...
        .then((db: SQLiteObject) => {
          db.executeSql(
            ...
            "promo, outlet_type, outlet_name, week, purchase_time, day, panel_code, last, year, period) " +
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            [
              data.stored,
              data.prod_cat,
      ...
      this.sqlite
        ...
        .then((db: SQLiteObject) => {
          db.executeSql(
            ...
            "gender=?,age=?,quantity=?,price=?,promo=?,outlet_type=?,outlet_name=?,week=?,purchase_time=?,day=?,panel_code=?,year=?,period=?" +
            "WHERE rowId=?",
            [
              data.stored,
              data.prod_cat,
